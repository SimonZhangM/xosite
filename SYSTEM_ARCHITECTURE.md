...
除了 MCP 服务本身，当前仓库内还已经落地了一份 **repo-local XplorOne agent skill**，用于约束外部 agent 如何稳定使用这些只读 MCP tools。

当前位置：
- [D:\APP\finance-v1\.codex\skills\xplorone\SKILL.md](/D:/APP/finance-v1/.codex/skills/xplorone/SKILL.md)
- [D:\APP\finance-v1\.codex\skills\xplorone\references](/D:/APP/finance-v1/.codex/skills/xplorone/references)
...
- 常规手工录入流水
- 单页录入体验
- 单页中的账户 / 分类双栏下拉当前已经改为 body portal 浮层，不再依赖表单容器内 `absolute` 展开。
...
其中 `ReportMemberPage` 当前已不只是原来的基础成员页，而是已形成一套独立工作台：
- 顶部汇总条
- 左侧本月收入 / 支出排行榜
- 中部月历主体
- 右侧当日收支排行榜
...
其中 `AccountManagementPage` 当前已切到与 `HomeLedgerPage` 接近的工作台式布局：
- 顶部净资产汇总条
- 左侧时间筛选侧栏
- 右侧账户列表与资产 / 负债子视图切换
- 新建 / 编辑账户仍复用原账户服务链路，不单独拆后端模型

`CategoryManagementPage` 当前也已切到同一套新版工作台骨架：
- 顶部汇总条
- 左侧时间筛选侧栏 + 二级时间筛选
- 右侧分类列表与收入 / 支出子视图切换

`BudgetManagementPage` 当前也已切到同一套新版工作台骨架，但保留预算页自己的结构差异：
- 顶部余额 / 当月预算 / 实际支出汇总条
- 左侧年份预算侧栏
- 右侧预算列表、保存动作与预算状态框

`MemberManagementPage` 当前已经形成另一种稳定的新版管理页结构：
- 最外层白卡
- 标题区操作条
- 内部列表壳
- 分组式表格
...
- [D:\APP\finance-v1\src\backend\mcp\localApiClient.ts](/D:/APP/finance-v1/src/backend/mcp/localApiClient.ts)
  - MCP 到本地 API 的客户端封装
- [D:\APP\finance-v1\.codex\skills\xplorone\SKILL.md](/D:/APP/finance-v1/.codex/skills/xplorone/SKILL.md)
  - repo-local XplorOne agent skill 主入口，约束外部 agent 如何更稳地使用只读 MCP
- [D:\APP\finance-v1\src\main\localApiManager.ts](/D:/APP/finance-v1/src/main/localApiManager.ts)
